...
export default {
  data () {
    return {
      ...
      headers: [
        ...
        { text: 'ชื่อ', value: 'bookingDataCustomerName' },
  ...
  methods: {
    ...
    async fetchRatings () {
      ...
      try {
        ...
        if (Array.isArray(response.data)) {
          ...
          await this.groupByRefId(response.data)
          this.isLoading = false
        } else {
    ...
    groupByRefId (data) {
      const uniqueRefIds = new Set() // สร้าง Set เพื่อเก็บ refId ที่ไม่ซ้ำกัน
      this.ratingsgroup = data.filter(item => {
        if (!uniqueRefIds.has(item.refId)) {
          uniqueRefIds.add(item.refId) // เพิ่ม refId ลงใน Set
          return true // เก็บ item นี้
        }
        return false // ไม่เก็บ item นี้
      })
    },
    groupByRefd (ref) {
      console.log('refId', ref)
      this.ratingsgroupByrefId = this.ratings.filter(item => {
        return item.refId === ref // คืนค่า item ถ้า refId ตรงกัน
      })
      console.log('this.ratingsgroupByrefId', this.ratingsgroupByrefId)
    },
  ...
  <v-app>
    <v-main>
      ...
      <v-app-bar flat class="app-bar" dark>
        ...
        <v-badge overlap color="success" dot offset-x="12" offset-y="40">
          <v-avatar size="42">
            <v-img :src="userPhotoURL" alt="User Profile"
              @error="userPhotoURL = 'https://upload.wikimedia.org/wikipedia/commons/7/7c/Profile_avatar_placeholder_large.png?20150327203541'"></v-img>
      ...
      <v-container fluid>
        <v-row align="center">
          ...
          <v-col>
            <v-card-text class="text-h5 mb-5 mt-4 h2 font-weight-bold" style="color: #303030;">My Rating</v-card-text>
          </v-col>
        </v-row>
        ...
        <!-- Data Table -->
        <v-data-table v-else :headers="headers" :items="ratingsgroup" class="elevation-1">
          <!-- สำหรับการแสดงชื่อและรูปของลูกค้า -->
          <template slot="item.bookingDataCustomerName" slot-scope="{ item }">
            <v-layout align-center>
              <v-avatar size="32" class="mr-2">
                <v-img :src="item.pictureUrl" alt="Customer Photo" @error="() => handleImageError(item)"></v-img>
              </v-avatar>
              <span>{{ item.bookingDataCustomerName }}</span>
            </v-layout>
          </template>

          <!-- สำหรับวันที่สร้าง -->
          <template slot="item.CREATE_DATE" slot-scope="{ item }">
            <span>{{ item.CREATE_DATE }}</span>
          </template>

          <!-- สำหรับการให้ดาว rating -->
          <template slot="item.rating" slot-scope="{ item }">
            <v-icon v-for="star in 5" :key="star" :color="star <= item.rating ? 'warning' : 'grey'">
              mdi-star
            </v-icon>
          </template>
          ...
          <template slot="item.actions" slot-scope="{ item }">
            <!-- {{slotProps.item  }} -->
            <v-btn @click="openDialog(item), groupByRefd(item.refId)" color="secondary" small outlined
              style="font-size: 0.8rem; padding: 5px 10px;">
              <span class="text-caption font-weight-bold">รายละเอียด</span>
            </v-btn>
            <!-- Dialog -->
              ...
              <v-card style="border-radius: 5px;">
                ...
                <v-card-text class="py-0">
                  <v-container class="pa-12"> <!-- เพิ่ม padding -->
                    <v-row>
                      <v-col class="py-1" cols="12" style="color: black;">
                        <v-icon color="#043873" left class="align-top">mdi-calendar</v-icon> <!-- ไอคอนวันที่ -->
                        <strong style="color: #043873;">วันที่นัดหมาย :</strong>
                        <span class="font-weight-bold">{{ selectedItem.dueDate }}</span>
                      </v-col>
                    </v-row>
                    <v-row>
                      <v-col class="py-1" cols="12">
                        <v-icon color="#043873" left class="align-top">mdi-account</v-icon> <!-- ไอคอนชื่อ -->
                        <strong style="font-weight: bold; color: black; color: #043873;">ชื่อ :</strong>
                        <span class="font-weight-light"
                          style="color: #1e1e1e;">{{ selectedItem.bookingDataCustomerName }}</span>
                      </v-col>
                    </v-row>
                    ...
                    <v-row>
                      <v-col class="py-1" cols="12">
                        ...
                        <strong style="font-weight: bold; color: #043873;">เบอร์โทร :</strong>
                        <span style="font-weight: lighter; color: #1e1e1e;">{{ selectedItem.bookingDataCustomerTel
                          }}</span>
                      </v-col>
                    </v-row>
                    ...
                    <!-- คะแนนที่ได้รับ -->
                    <v-row>
                      <v-col class="py-1 mt-2 mb-2" cols="12" style="font-weight: bold; color: black;">
                        <strong>คะแนนที่ได้</strong>
                      </v-col>
                    </v-row>
                    <!-- {{ selectItem }} -->
                    <div v-if="ratingItem.refId">
                      <v-row>
                        <v-col cols="12" class="py-0">
                          พึงพอใจสถานที่เข้ารับบริการ
                        </v-col>
                        <v-col cols="12" class="py-0">
                          <v-rating :value="Number(selectedItem.rating)" color="warning" empty-icon-color="grey" dense
                            readonly></v-rating>
                        </v-col>
                      </v-row>
                    </div>
                    <div v-else>
                      <v-row v-for="item in ratingsgroupByrefId" :key="item.id">
                        <v-col cols="12" class="py-0">
                          {{ item.answer }}
                        </v-col>
                        <v-col cols="12" class="py-0">
                          <v-rating :value="Number(item.rating)" color="warning" empty-icon-color="grey" dense
                            readonly></v-rating>
                        </v-col>
                      </v-row>
                    </div>
                    ...
                    <v-row class="mt-4">
                      <v-col cols="12" class="py-2">
                        ...
                        <strong style="font-weight: bold; color: #043873">ความคิดเห็นเพิ่มเติม :</strong>
                        <span style="font-weight: lighter; color: #1e1e1e;">{{ selectedItem.comment }}</span>
                      </v-col>
                    </v-row>
                  </v-container>
                </v-card-text>
